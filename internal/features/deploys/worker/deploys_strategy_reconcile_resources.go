...
func applyServiceWorkloadResources(
	...
	deps := Dependencies{
		...
		FetchResourceFn: func(ctx context.Context, client *http.Client, token, apiVersion, kind, namespace, name string) (map[string]interface{}, error) {
			return platformkube.FetchResourceAsMap(ctx, client, token, apiVersion, kind, namespace, name)
		},
...
func reconcileStrategyResources(
	...
	if strategyType == "canary" {
		stableExists, err := platformkube.ResourceExists(ctx, client, token, "apps/v1", "Deployment", namespace, serviceName)
		if err != nil {
		...
		baseDeployment, err := platformkube.FetchResourceAsMap(ctx, client, token, "apps/v1", "Deployment", namespace, serviceName)
		if err != nil {
			return fmt.Errorf("strategy reconcile: %w", err)
		}
		baseService, err := platformkube.FetchResourceAsMap(ctx, client, token, "v1", "Service", namespace, serviceName)
		if err != nil {
			return fmt.Errorf("strategy reconcile: %w", err)
		}
		platformkube.CleanResourceForReapply(baseDeployment)
		platformkube.CleanResourceForReapply(baseService)
		...
		output, undoErr := platformutils.RunCommandWithInput(ctx, "kubectl", undoArgs, "")
		if undoErr != nil {
		...
		patchOutput, patchErr := platformutils.RunCommandWithInput(ctx, "kubectl", patchArgs, "")
		if patchErr != nil {
