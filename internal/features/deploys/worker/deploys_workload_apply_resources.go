...
func stampDeployRevisionKubectl(ctx context.Context, cfg models.Config, environment, serviceName string, service models.ServiceConfig, logger *platformlogging.DeployLogger) {
	if serviceName == "" {
		return
	}
	...
	output, err := platformutils.RunCommandWithInput(ctx, "kubectl", args, "")
	if err != nil {
		if logger != nil {
...
func applyResourcesYAML(ctx context.Context, resourcesYAML string, logger *platformlogging.DeployLogger) error {
	clean := strings.TrimSpace(resourcesYAML)
	if clean == "" {
		return errors.New("deploy resources yaml missing")
	}
	output, err := platformutils.RunCommandWithInput(ctx, "kubectl", []string{"apply", "-f", "-"}, clean)
	if output != "" {
		if logger != nil {
...
func applyRenderedResources(ctx context.Context, cfg models.Config, resources []map[string]interface{}, environment string, ctxData models.DeployContext, logger *platformlogging.DeployLogger) error {
	if len(resources) == 0 {
	...
	for namespace := range namespaces {
		if err := platformkube.EnsureNamespace(ctx, client, token, namespace); err != nil {
			return err
		}
	}
	firstCanaryDeploy, firstCanaryErr := detectFirstCanaryDeploy(ctx, client, token, defaultNamespace, serviceName, resources, canaryOnly)
	if firstCanaryErr != nil {
		return firstCanaryErr
	}
	...
	for _, resource := range resources {
		...
		if canaryOnly && serviceName != "" && name == serviceName && (strings.EqualFold(kind, "Deployment") || strings.EqualFold(kind, "Service")) {
			if firstCanaryDeploy {
				...
				if stableResource != nil {
					...
					if err := platformkube.ApplyResource(ctx, client, token, stableResource); err != nil {
						return err
					}
					if logger != nil {
						logger.Flush(ctx)
			...
			if canaryResource != nil {
				...
				if err := platformkube.ApplyResource(ctx, client, token, canaryResource); err != nil {
					return err
				}
				if logger != nil {
					logger.Flush(ctx)
		...
		if err := platformkube.ApplyResource(ctx, client, token, resource); err != nil {
			return err
		}
		if logger != nil {
			logger.Flush(ctx)
...
func detectFirstCanaryDeploy(
	...
	stableExists, err := platformkube.ResourceExists(ctx, client, token, "apps/v1", "Deployment", stableNamespace, serviceName)
	if err != nil {
		return false, err
	}
	return !stableExists, nil
}

func applyDeployResources(ctx context.Context, cfg models.Config, ctxData models.DeployContext, environment string, logger *platformlogging.DeployLogger) error {
	...
	if len(secretEnv) > 0 {
		...
		if err := platformkube.ApplyResource(ctx, client, token, secretResource); err != nil {
			return err
		}
		if logger != nil {
			logger.Flush(ctx)
