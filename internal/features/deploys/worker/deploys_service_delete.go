...
func HandleServiceDelete(ctx context.Context, client *http.Client, cfg models.Config, tokens *platformauth.TokenManager, op models.OperationPayload) error {
	...
	kubeClient, token, err := platformkube.KubeClient()
	if err != nil {
		return err
	}
	...
	for _, name := range workloadNames {
		if err := platformkube.DeleteResource(ctx, kubeClient, token, "apps/v1", "Deployment", namespace, name); err != nil {
			return err
		}
		if err := platformkube.DeleteResource(ctx, kubeClient, token, "v1", "Service", namespace, name); err != nil {
			return err
		}
	}
	if err := platformkube.DeleteResource(ctx, kubeClient, token, "batch/v1", "CronJob", namespace, serviceName); err != nil {
		return err
	}
	if err := platformkube.DeleteResourcesByLabel(ctx, kubeClient, token, "batch/v1", "Job", namespace, fmt.Sprintf("app=%s", serviceName)); err != nil {
		return err
	}
	if err := platformkube.DeleteResource(ctx, kubeClient, token, "autoscaling/v2", "HorizontalPodAutoscaler", namespace, serviceName); err != nil {
		return err
	}
	if err := platformkube.DeleteResource(ctx, kubeClient, token, "v1", "Secret", namespace, serviceName); err != nil {
		return err
	}
	if err := platformkube.DeleteResource(ctx, kubeClient, token, "v1", "ConfigMap", namespace, serviceName); err != nil {
		return err
	}
	if err := platformkube.DeleteResourcesByLabel(ctx, kubeClient, token, "networking.istio.io/v1beta1", "VirtualService", namespace, fmt.Sprintf("releasea.service=%s", ctxData.Service.ID)); err != nil {
		return err
	}
	if err := platformkube.DeleteResourcesByLabel(ctx, kubeClient, token, "security.istio.io/v1beta1", "AuthorizationPolicy", namespace, fmt.Sprintf("releasea.service=%s", ctxData.Service.ID)); err != nil {
		return err
	}
...
func fetchServiceContext(ctx context.Context, client *http.Client, cfg models.Config, tokens *platformauth.TokenManager, serviceID, environment string) (models.DeployContext, error) {
	if serviceID == "" {
