...
func handleStaticSiteDeploy(ctx context.Context, cfg models.Config, ctxData models.DeployContext, environment string, logger *platformlogging.DeployLogger) error {
	...
	if installCommand := strings.TrimSpace(ctxData.Service.InstallCommand); installCommand != "" {
		...
		if err := platformutils.RunShellWithLogger(ctx, workDir, installCommand, logger); err != nil {
			return err
		}
		if logger != nil {
			logger.Flush(ctx)
	...
	if buildCommand := strings.TrimSpace(ctxData.Service.BuildCommand); buildCommand != "" {
		...
		if err := platformutils.RunShellWithLogger(ctx, workDir, buildCommand, logger); err != nil {
			return err
		}
		if logger != nil {
			logger.Flush(ctx)
...
func staticSitePrefix(cfg models.Config, svc models.ServiceConfig) string {
	name := shared.ToKubeName(svc.Name)
	if name == "" {
		name = shared.ToKubeName(svc.ID)
	}
	if name == "" {
		name = "site"
	}
...
func ensureMinioAlias(ctx context.Context, cfg models.Config, logger *platformlogging.DeployLogger) (string, error) {
	...
	if err := platformutils.RunCommandWithLogger(ctx, "", "mc", args, nil, logger); err != nil {
		return "", err
	}
	return alias, nil
}

func ensureMinioBucket(ctx context.Context, alias, bucket string, logger *platformlogging.DeployLogger) error {
	target := fmt.Sprintf("%s/%s", alias, bucket)
	args := []string{"mb", "--ignore-existing", target}
	if logger != nil {
		logger.Logf(ctx, "ensuring bucket %s", target)
	}
	return platformutils.RunCommandWithLogger(ctx, "", "mc", args, nil, logger)
}

func setBucketPublic(ctx context.Context, alias, bucket string, logger *platformlogging.DeployLogger) error {
	target := fmt.Sprintf("%s/%s", alias, bucket)
	if logger != nil {
		logger.Logf(ctx, "setting bucket policy to public read (%s)", target)
	}
	if err := platformutils.RunCommandWithLogger(ctx, "", "mc", []string{"anonymous", "set", "download", target}, nil, logger); err == nil {
		return nil
	}
	return platformutils.RunCommandWithLogger(ctx, "", "mc", []string{"policy", "set", "download", target}, nil, logger)
}

func mirrorStaticSite(ctx context.Context, alias, bucket, prefix, dir, cacheControl string, logger *platformlogging.DeployLogger) error {
...
func deleteStaticSiteAssets(ctx context.Context, cfg models.Config, svc models.ServiceConfig) error {
	...
	return platformutils.RunCommandWithLogger(ctx, "", "mc", args, nil, nil)
}
