...
var errDeploymentNotFound = platformkube.ErrDeploymentNotFound

func RunCurator(ctx context.Context, cfg models.Config, tokens *platformauth.TokenManager) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err := curateDeployments(cycleCtx, client, cfg, tokens, maxAge); err != nil {
				log.Printf("[worker] curator error: %v", err)
			}
			if err := curateRuleDeploys(cycleCtx, client, cfg, tokens, maxAge); err != nil {
				log.Printf("[worker] curator error: %v", err)
			}
			cancel()
		}
...
func curateDeployments(ctx context.Context, client *http.Client, cfg models.Config, tokens *platformauth.TokenManager, maxAge time.Duration) error {
	ops, err := platformops.FetchOperationsByStatus(ctx, client, cfg, tokens, models.OperationStatusInProgress, models.OperationTypeServiceDeploy)
	if err != nil {
		return err
	}
	if len(ops) == 0 {
	...
	for _, op := range ops {
		env := platformops.PayloadString(op.Payload, "environment")
		if env == "" {
			env = "prod"
		}
		...
		for _, target := range targets {
			...
			if statusErr != nil {
				if errors.Is(statusErr, errDeploymentNotFound) {
					...
					if opAgeExceeded(op, maxAge) {
						_ = platformops.UpdateOperationStatus(ctx, client, cfg, tokens, op.ID, models.OperationStatusFailed, fmt.Sprintf("deployment %s not found", target))
						failed = true
						break
					}
					continue
			...
			if deployFailed {
				_ = platformops.UpdateOperationStatus(ctx, client, cfg, tokens, op.ID, models.OperationStatusFailed, fmt.Sprintf("%s: %s", target, reason))
				failed = true
				break
			}
			if !ready {
			...
			if podFailed {
				_ = platformops.UpdateOperationStatus(ctx, client, cfg, tokens, op.ID, models.OperationStatusFailed, podReason)
				failed = true
				break
			}
		}
		...
		if allReady {
			_ = platformops.UpdateOperationStatus(ctx, client, cfg, tokens, op.ID, models.OperationStatusSucceeded, "")
		}
	}
	return nil
}

func curateRuleDeploys(ctx context.Context, client *http.Client, cfg models.Config, tokens *platformauth.TokenManager, maxAge time.Duration) error {
	ops, err := platformops.FetchOperationsByStatus(ctx, client, cfg, tokens, models.OperationStatusInProgress, models.OperationTypeRuleDeploy)
	if err != nil {
		return err
	}
	if len(ops) == 0 {
	...
	for _, op := range ops {
		...
		if err != nil {
			if opAgeExceeded(op, maxAge) {
				_ = platformops.UpdateOperationStatus(ctx, client, cfg, tokens, op.ID, models.OperationStatusFailed, "rule fetch failed")
			}
			continue
		}
		...
		if err != nil {
			if opAgeExceeded(op, maxAge) {
				_ = platformops.UpdateOperationStatus(ctx, client, cfg, tokens, op.ID, models.OperationStatusFailed, "service fetch failed")
			}
			continue
		}
		...
		if rulesmodule.RuleAction(rule) == "deny" {
			...
			exists, err := platformkube.ResourceExists(ctx, kubeClient, kubeToken, "security.istio.io/v1beta1", "AuthorizationPolicy", namespace, policyName)
			if err != nil {
				if opAgeExceeded(op, maxAge) {
					_ = platformops.UpdateOperationStatus(ctx, client, cfg, tokens, op.ID, models.OperationStatusFailed, "authorization policy check failed")
				}
				continue
			}
			if exists {
			...
			if opAgeExceeded(op, maxAge) {
				_ = platformops.UpdateOperationStatus(ctx, client, cfg, tokens, op.ID, models.OperationStatusFailed, "authorization policy missing")
			}
			continue
		}

		vsName := rulesmodule.RuleVirtualServiceName(serviceName, rule.ID)
		exists, err := platformkube.ResourceExists(ctx, kubeClient, kubeToken, "networking.istio.io/v1beta1", "VirtualService", namespace, vsName)
		if err != nil {
			if opAgeExceeded(op, maxAge) {
				_ = platformops.UpdateOperationStatus(ctx, client, cfg, tokens, op.ID, models.OperationStatusFailed, "virtual service check failed")
			}
			continue
		}
		if exists {
		...
		if opAgeExceeded(op, maxAge) {
			_ = platformops.UpdateOperationStatus(ctx, client, cfg, tokens, op.ID, models.OperationStatusFailed, "virtual service missing")
		}
	}
	return nil
}
