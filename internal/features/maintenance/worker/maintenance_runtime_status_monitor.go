...
func RunRuntimeMonitor(ctx context.Context, cfg models.Config, tokens *platformauth.TokenManager) {
	interval := time.Duration(shared.EnvInt("WORKER_RUNTIME_SECONDS", defaultRuntimeMonitorSeconds)) * time.Second
	if interval <= 0 {
	...
	initialCtx, cancelInitial := context.WithTimeout(ctx, resolveMaintenanceCycleTimeout(interval))
	if err := UpdateRuntimeStatuses(initialCtx, client, cfg, tokens); err != nil {
	...
	for {
		...
		case <-ticker.C:
			...
			if err := UpdateRuntimeStatuses(cycleCtx, client, cfg, tokens); err != nil {
...
func UpdateRuntimeStatuses(ctx context.Context, client *http.Client, cfg models.Config, tokens *platformauth.TokenManager) error {
...
func resumeReplicas(service models.ServicePayload) int {
...
func resolvePauseIdleWindow(service models.ServicePayload) time.Duration {
	timeoutSeconds := service.PauseIdleTimeoutSeconds
	if timeoutSeconds <= 0 {
		timeoutSeconds = shared.EnvInt("WORKER_PAUSE_IDLE_DEFAULT_SECONDS", defaultPauseIdleTimeoutSeconds)
	}
...
func resolvePauseResumeWindow(idleWindow time.Duration) time.Duration {
	resumeSeconds := shared.EnvInt("WORKER_PAUSE_IDLE_RESUME_WINDOW_SECONDS", defaultPauseIdleResumeWindowSeconds)
...
func fetchServices(ctx context.Context, client *http.Client, cfg models.Config, tokens *platformauth.TokenManager) ([]models.ServicePayload, error) {
...
func updateServiceRuntime(ctx context.Context, client *http.Client, cfg models.Config, tokens *platformauth.TokenManager, serviceID, environment, status, reason string) error {
...
func detectRuntimeIssue(pods models.PodList) runtimeIssue {
	issue := runtimeIssue{}
	if len(pods.Items) == 0 {
		return issue
	}
...
func inspectPodRuntime(pod models.PodInfo) runtimeIssue {
