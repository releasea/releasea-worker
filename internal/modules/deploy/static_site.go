...
func handleStaticSiteDeploy(ctx context.Context, cfg models.Config, ctxData models.DeployContext, environment string, logger *platform.DeployLogger) error {
	...
	if installCommand := strings.TrimSpace(ctxData.Service.InstallCommand); installCommand != "" {
		...
		if err := platform.RunShellWithLogger(ctx, workDir, installCommand, logger); err != nil {
			return err
		}
		if logger != nil {
			logger.Flush(ctx)
	...
	if buildCommand := strings.TrimSpace(ctxData.Service.BuildCommand); buildCommand != "" {
		...
		if err := platform.RunShellWithLogger(ctx, workDir, buildCommand, logger); err != nil {
			return err
		}
		if logger != nil {
			logger.Flush(ctx)
...
func ensureMinioAlias(ctx context.Context, cfg models.Config, logger *platform.DeployLogger) (string, error) {
	...
	if err := platform.RunCommandWithLogger(ctx, "", "mc", args, nil, logger); err != nil {
		return "", err
	}
	return alias, nil
}

func ensureMinioBucket(ctx context.Context, alias, bucket string, logger *platform.DeployLogger) error {
	target := fmt.Sprintf("%s/%s", alias, bucket)
	args := []string{"mb", "--ignore-existing", target}
	if logger != nil {
		logger.Logf(ctx, "ensuring bucket %s", target)
	}
	return platform.RunCommandWithLogger(ctx, "", "mc", args, nil, logger)
}

func setBucketPublic(ctx context.Context, alias, bucket string, logger *platform.DeployLogger) error {
	target := fmt.Sprintf("%s/%s", alias, bucket)
	if logger != nil {
		logger.Logf(ctx, "setting bucket policy to public read (%s)", target)
	}
	if err := platform.RunCommandWithLogger(ctx, "", "mc", []string{"anonymous", "set", "download", target}, nil, logger); err == nil {
		return nil
	}
	return platform.RunCommandWithLogger(ctx, "", "mc", []string{"policy", "set", "download", target}, nil, logger)
}

func mirrorStaticSite(ctx context.Context, alias, bucket, prefix, dir, cacheControl string, logger *platform.DeployLogger) error {
...
func deleteStaticSiteAssets(ctx context.Context, cfg models.Config, svc models.ServiceConfig) error {
	...
	return platform.RunCommandWithLogger(ctx, "", "mc", args, nil, nil)
}
