...
func runPoller(ctx context.Context, cfg models.Config, tokens *platform.TokenManager) {
...
func drainQueuedOperations(ctx context.Context, client *http.Client, cfg models.Config, tokens *platform.TokenManager, limit int) error {
	ops, err := platform.FetchQueuedOperations(ctx, client, cfg, tokens)
	if err != nil {
		return err
	}
	if len(ops) == 0 {
