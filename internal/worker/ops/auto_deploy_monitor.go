...
func runAutoDeployMonitor(ctx context.Context, cfg Config, tokens *tokenManager) {
	if envInt("WORKER_AUTODEPLOY_ENABLED", 1) == 0 {
		log.Printf("[worker] auto deploy monitor disabled")
		return
	}

	interval := time.Duration(envInt("WORKER_AUTODEPLOY_SECONDS", 60)) * time.Second
	...
	runCycle := func() {
		if err := runAutoDeployCycle(ctx, client, cfg, tokens, runtime, interval, leaseTTLSeconds); err != nil {
...
func resolveAutoDeployLeaseTTL(interval time.Duration) int {
	...
	ttl := envInt("WORKER_AUTODEPLOY_LEASE_SECONDS", defaultTTL)
	if ttl < 30 {
		ttl = 30
	}
	if ttl > 600 {
...
func resolveAutoDeployErrorCooldown(interval time.Duration) time.Duration {
	seconds := envInt("WORKER_AUTODEPLOY_ERROR_COOLDOWN_SECONDS", int(interval.Seconds()))
	if seconds < 20 {
		seconds = 20
	}
	if seconds > 900 {
...
func resolveAutoDeployQueueErrorCooldown(interval time.Duration) time.Duration {
	seconds := envInt("WORKER_AUTODEPLOY_QUEUE_ERROR_COOLDOWN_SECONDS", int(interval.Seconds()/2))
	if seconds < 10 {
		seconds = 10
	}
	if seconds > 300 {
...
func resolveAutoDeployRecentQueueTTL(interval time.Duration) time.Duration {
	seconds := envInt("WORKER_AUTODEPLOY_PENDING_SECONDS", int(interval.Seconds()*2)+10)
	if seconds < 30 {
		seconds = 30
	}
	if seconds > 900 {
...
func shouldAutoDeployService(service servicePayload) bool {
...
func fetchAutoDeployServices(ctx context.Context, client *http.Client, cfg Config, tokens *tokenManager) ([]servicePayload, error) {
...
func fetchDeploySnapshots(ctx context.Context, client *http.Client, cfg Config, tokens *tokenManager) ([]deploySnapshot, error) {
