...
func runConsumer(ctx context.Context, cfg models.Config, tokens *platform.TokenManager) error {
	conn, err := platform.DialRabbitMQ(cfg.RabbitURL)
	if err != nil {
		return err
	}
	defer conn.Close()
...
func processJob(ctx context.Context, client *http.Client, cfg models.Config, tokens *platform.TokenManager, msg amqp.Delivery) error {
