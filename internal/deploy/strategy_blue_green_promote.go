...
func promoteBlueGreen(
	...
	if err := platform.UpdateBlueGreenActiveSlot(ctx, client, cfg, tokens, serviceID, environment, candidateSlot); err != nil {
	...
	if err := waitForServiceDeployReadiness(
		...
		_ = switchBlueGreenCanonicalService(ctx, kubeClient, kubeToken, namespace, serviceName, activeName)
		_ = platform.UpdateBlueGreenActiveSlot(ctx, client, cfg, tokens, serviceID, environment, activeSlot)
...
func switchBlueGreenCanonicalService(
	...
	canonicalService, err := platform.FetchResourceAsMap(ctx, kubeClient, kubeToken, "v1", "Service", namespace, serviceName)
	if err != nil {
	...
	return platform.ApplyResource(ctx, kubeClient, kubeToken, canonicalService)
}
...
func cleanupBlueGreenSlot(
	...
	if err := platform.DeleteResource(ctx, kubeClient, kubeToken, "apps/v1", "Deployment", namespace, slotName); err != nil {
		return err
	}
	if err := platform.DeleteResource(ctx, kubeClient, kubeToken, "v1", "Service", namespace, slotName); err != nil {
		return err
	}
	if logger != nil {
...
func cleanupLegacyRollingWorkload(
	...
	if err := platform.DeleteResource(ctx, kubeClient, kubeToken, "apps/v1", "Deployment", namespace, serviceName); err != nil {
		return err
	}
	if err := platform.DeleteResource(ctx, kubeClient, kubeToken, "autoscaling/v2", "HorizontalPodAutoscaler", namespace, serviceName); err != nil {
		return err
	}
	if logger != nil {
