...
func normalizeWorkloadStrategyType(service ServiceConfig) string {
...
func resolveCanaryPercent(service ServiceConfig) int {
	return shared.NormalizeCanaryPercent(service.DeploymentStrategy.CanaryPercent)
}
...
func resolvePrimaryBlueGreenColor(service ServiceConfig) string {
	primary, _ := shared.ResolveBlueGreenSlots(service.DeploymentStrategy.BlueGreenPrimary)
	return primary
}

func oppositeBlueGreenColor(primary string) string {
	_, secondary := shared.ResolveBlueGreenSlots(primary)
	return secondary
}
...
func syncAutoscalerResource(
	...
	if normalizeWorkloadStrategyType(service) == "canary" || normalizeWorkloadStrategyType(service) == "blue-green" {
		return deps.DeleteResourceFn(ctx, client, token, "autoscaling/v2", "HorizontalPodAutoscaler", namespace, serviceName)
	}
	minReplicas := service.MinReplicas
	if minReplicas < 1 {
