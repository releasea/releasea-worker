...
func buildStrategyDestinations(service servicePayload, serviceName, namespace string, port int) []interface{} {
	...
	case "blue-green":
		primaryColor, _ := resolveBlueGreenSlots(service.DeploymentStrategy.BlueGreenPrimary)
...
func buildDenyPolicyName(serviceName, ruleName, ruleID string) string {
	baseService := toKubeName(serviceName)
	if baseService == "" {
		baseService = "service"
	}
	baseRule := toKubeName(ruleName)
	if baseRule == "" {
		baseRule = toKubeName(ruleID)
	}
	if baseRule == "" {
		baseRule = "rule"
	}
...
func resolveRuleHosts(hosts []string, gateways []string, serviceName, namespace string, cfg Config) []string {
	out := uniqueStrings(hosts)
	if gatewayMatches(gateways, cfg.InternalGateway) && cfg.InternalDomain != "" {
		if !hasHostSuffix(out, cfg.InternalDomain) {
	...
	if gatewayMatches(gateways, cfg.ExternalGateway) && cfg.ExternalDomain != "" {
		if !hasHostSuffix(out, cfg.ExternalDomain) {
...
func staticNginxHost(cfg Config) string {
