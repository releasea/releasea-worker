...
func HandleRuleDeploy(ctx context.Context, client *http.Client, cfg models.Config, tokens *platform.TokenManager, op models.OperationPayload) error {
	if op.Resource == "" {
		return errors.New("rule id missing")
	}
	logger := platform.NewRuleDeployLogger(client, cfg, tokens, op.RuleDeployID)
	if logger != nil {
	...
	if RuleAction(rule) == "deny" {
		...
		if err := platform.EnsureNamespace(ctx, kubeClient, kubeToken, namespace); err != nil {
			return err
		}
		if len(gateways) == 0 {
			...
			return platform.DeleteResource(ctx, kubeClient, kubeToken, "security.istio.io/v1beta1", "AuthorizationPolicy", namespace, policyName)
		}
		...
		return platform.ApplyResource(ctx, kubeClient, kubeToken, policy)
	}
	...
	if len(gateways) == 0 {
		...
		if err := platform.DeleteResource(ctx, kubeClient, kubeToken, "networking.istio.io/v1beta1", "VirtualService", namespace, vsName); err != nil {
			return err
		}
		return nil
	}
	...
	if err := platform.ApplyResource(ctx, kubeClient, kubeToken, vs); err != nil {
		return err
	}
...
func HandleRuleDelete(ctx context.Context, client *http.Client, cfg models.Config, tokens *platform.TokenManager, op models.OperationPayload) error {
	if op.Resource == "" {
		return errors.New("rule id missing")
	}
	...
	environment := strings.TrimSpace(platform.PayloadString(op.Payload, "environment"))
	if environment == "" {
		environment = "prod"
	}

	serviceName := strings.TrimSpace(platform.PayloadString(op.Payload, "serviceName"))
	serviceID := strings.TrimSpace(platform.PayloadString(op.Payload, "serviceId"))
	...
	action := strings.ToLower(strings.TrimSpace(platform.PayloadString(op.Payload, "action")))
	if action == "deny" {
		ruleName := strings.TrimSpace(platform.PayloadString(op.Payload, "ruleName"))
	...
	return platform.DeleteResource(ctx, kubeClient, kubeToken, "networking.istio.io/v1beta1", "VirtualService", namespace, vsName)
}

func FetchRule(ctx context.Context, client *http.Client, cfg models.Config, tokens *platform.TokenManager, ruleID string) (models.RulePayload, error) {
	var rule models.RulePayload
	err := platform.DoJSONRequest(
		ctx,
		client,
		cfg,
		tokens,
...
func FetchService(ctx context.Context, client *http.Client, cfg models.Config, tokens *platform.TokenManager, serviceID string) (models.ServicePayload, error) {
...
func applyRuleDeployStrategyOverride(service models.ServicePayload, payload map[string]interface{}) models.ServicePayload {
	...
	override := platform.PayloadInt(payload, "canaryPercentOverride")
	if override < 0 {
		override = 0
	}
	if override > 50 {
