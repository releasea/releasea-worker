...
func normalizeRuleDeployStrategyForAvailability(
	...
	case "canary":
		...
		return service
	case "blue-green":
		primaryColor, secondaryColor := deploymodule.ResolveBlueGreenSlots(service.DeploymentStrategy.BlueGreenPrimary)
		...
		canonicalServiceExists, err := platform.ResourceExists(ctx, client, token, "v1", "Service", namespace, serviceName)
...
func workloadTargetReady(
	...
	serviceReady, err := platform.ResourceExists(ctx, client, token, "v1", "Service", namespace, workloadName)
...
func resolveDeployStrategyTypeForServicePayload(service models.ServicePayload) string {
	return deploymodule.ResolveDeployStrategyType(models.ServiceConfig{
		Type:             service.Type,
