...
func runCurator(ctx context.Context, cfg Config, tokens *tokenManager) {
	interval := time.Duration(envInt("WORKER_CURATOR_SECONDS", 60)) * time.Second
	if interval <= 0 {
		interval = 60 * time.Second
	}
	maxAge := time.Duration(envInt("WORKER_CURATOR_MAX_SECONDS", 600)) * time.Second
	if maxAge <= 0 {
...
func curateDeployments(ctx context.Context, client *http.Client, cfg Config, tokens *tokenManager, maxAge time.Duration) error {
	ops, err := fetchOperationsByStatus(ctx, client, cfg, tokens, "in-progress", "service.deploy")
	if err != nil {
		return err
	}
	if len(ops) == 0 {
	...
	for _, op := range ops {
		env := payloadString(op.Payload, "environment")
		if env == "" {
			env = "prod"
		}
		...
		if op.Resource != "" {
			fetched, err := fetchService(ctx, client, cfg, tokens, op.Resource)
			if err == nil {
				service = fetched
				serviceFetched = true
				if !isWorkloadReadinessRequired(service.Type, service.DeployTemplateID) {
					continue
				}
			}
		}
		name := toKubeName(op.ServiceName)
		if name == "" && serviceFetched {
			name = toKubeName(service.Name)
		}
		if name == "" {
			name = toKubeName(op.Resource)
		}
		if name == "" {
			continue
		}
		namespace := resolveDeployNamespaceFromPayload(op.Payload, resolveNamespace(cfg, env))

		targets := []string{name}
		if serviceFetched {
			resolvedTargets := resolveServicePayloadDeployReadinessTargets(service, name, op.Payload)
		...
		for _, target := range targets {
			...
			if statusErr != nil {
				if errors.Is(statusErr, errDeploymentNotFound) {
					...
					if opAgeExceeded(op, maxAge) {
						_ = updateOperationStatus(ctx, client, cfg, tokens, op.ID, "failed", fmt.Sprintf("deployment %s not found", target))
						failed = true
						break
					}
					continue
			...
			if deployFailed {
				_ = updateOperationStatus(ctx, client, cfg, tokens, op.ID, "failed", fmt.Sprintf("%s: %s", target, reason))
				failed = true
				break
			}
			if !ready {
			...
			if podFailed {
				_ = updateOperationStatus(ctx, client, cfg, tokens, op.ID, "failed", podReason)
				failed = true
				break
			}
		}
		...
		if allReady {
			_ = updateOperationStatus(ctx, client, cfg, tokens, op.ID, "succeeded", "")
		}
	}
	return nil
}

func curateRuleDeploys(ctx context.Context, client *http.Client, cfg Config, tokens *tokenManager, maxAge time.Duration) error {
	ops, err := fetchOperationsByStatus(ctx, client, cfg, tokens, "in-progress", "rule.deploy")
	if err != nil {
		return err
	}
	if len(ops) == 0 {
	...
	for _, op := range ops {
		...
		if err != nil {
			if opAgeExceeded(op, maxAge) {
				_ = updateOperationStatus(ctx, client, cfg, tokens, op.ID, "failed", "rule fetch failed")
			}
			continue
		}
		...
		if err != nil {
			if opAgeExceeded(op, maxAge) {
				_ = updateOperationStatus(ctx, client, cfg, tokens, op.ID, "failed", "service fetch failed")
			}
			continue
		}
		...
		namespace := resolveNamespace(cfg, env)
		...
		if ruleAction(rule) == "deny" {
			...
			exists, err := resourceExists(ctx, kubeClient, kubeToken, "security.istio.io/v1beta1", "AuthorizationPolicy", namespace, policyName)
			if err != nil {
				if opAgeExceeded(op, maxAge) {
					_ = updateOperationStatus(ctx, client, cfg, tokens, op.ID, "failed", "authorization policy check failed")
				}
				continue
			}
			if exists {
			...
			if opAgeExceeded(op, maxAge) {
				_ = updateOperationStatus(ctx, client, cfg, tokens, op.ID, "failed", "authorization policy missing")
			}
			continue
		}

		vsName := ruleVirtualServiceName(serviceName, rule.ID)
		exists, err := resourceExists(ctx, kubeClient, kubeToken, "networking.istio.io/v1beta1", "VirtualService", namespace, vsName)
		if err != nil {
			if opAgeExceeded(op, maxAge) {
				_ = updateOperationStatus(ctx, client, cfg, tokens, op.ID, "failed", "virtual service check failed")
			}
			continue
		}
		if exists {
		...
		if opAgeExceeded(op, maxAge) {
			_ = updateOperationStatus(ctx, client, cfg, tokens, op.ID, "failed", "virtual service missing")
		}
	}
	return nil
}

func fetchDeployment(ctx context.Context, client *http.Client, token, namespace, name string) (deploymentInfo, error) {
...
func evaluateDeploymentStatus(deploy deploymentInfo, op operationPayload, maxAge time.Duration) (bool, bool, string) {
...
func fetchPodsByLabel(ctx context.Context, client *http.Client, token, namespace, selector string) (podList, error) {
	var pods podList
...
func evaluatePodFailure(pod podInfo) (bool, string) {
...
func waitingFailureReason(waiting *containerStateWaiting) (bool, string) {
	if waiting == nil {
		return false, ""
	}
...
func terminatedFailureReason(term *containerStateTerminated) (bool, string) {
	if term == nil {
		return false, ""
	}
	if term.ExitCode == 0 {
...
func opAgeExceeded(op operationPayload, maxAge time.Duration) bool {
	if maxAge <= 0 {
		return false
	}
