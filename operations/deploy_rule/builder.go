...
import (
	...
	"releaseaworker/common/strutil"
	"releaseaworker/config"
	"releaseaworker/models"
)

func BuildVirtualService(rule models.RulePayload, service models.ServicePayload, serviceName, namespace string, cfg config.Config, gateways []string) map[string]interface{} {
	port := rule.Port
	if port <= 0 {
		if service.Port > 0 {
			port = service.Port
...
func BuildDenyPolicyName(serviceName, ruleName, ruleID string) string {
	baseService := strutil.ToKubeName(serviceName)
	if baseService == "" {
		baseService = "service"
	}
	baseRule := strutil.ToKubeName(ruleName)
	if baseRule == "" {
		baseRule = strutil.ToKubeName(ruleID)
	}
	if baseRule == "" {
		baseRule = "rule"
	}
...
func RuleVirtualServiceName(serviceName, ruleID string) string {
	base := strutil.ToKubeName(ruleID)
	if base == "" {
		base = "rule"
	}
...
func MapGateways(gateways []string, cfg config.Config) []string {
	...
	return strutil.UniqueStrings(out)
}
...
func buildStrategyDestinations(service models.ServicePayload, serviceName, namespace string, port int) []interface{} {
	...
	case "blue-green":
		primaryColor, _ := deploystrategy.ResolveBlueGreenSlots(service.DeploymentStrategy.BlueGreenPrimary)
...
func resolveRuleHosts(hosts []string, gateways []string, serviceName, namespace string, cfg config.Config) []string {
	out := strutil.UniqueStrings(hosts)
	if gatewayMatches(gateways, cfg.InternalGateway) && cfg.InternalDomain != "" {
		if !strutil.HasHostSuffix(out, cfg.InternalDomain) {
	...
	if gatewayMatches(gateways, cfg.ExternalGateway) && cfg.ExternalDomain != "" {
		if !strutil.HasHostSuffix(out, cfg.ExternalDomain) {
...
func staticNginxHost(cfg config.Config) string {
