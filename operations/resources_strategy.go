...
func applyServiceWorkloadResources(
	...
	deps := strategyworker.Dependencies{
		...
		FetchResourceFn: func(ctx context.Context, client *http.Client, token, apiVersion, kind, namespace, name string) (map[string]interface{}, error) {
			return fetchResourceAsMap(ctx, client, token, apiVersion, kind, namespace, name)
		},
		CloneResourceFn: func(resource map[string]interface{}) map[string]interface{} {
			return renderTemplateResource(resource, map[string]string{})
		},
	}
...
func reconcileStrategyResources(
	...
	if strategyType == "canary" {
		stableExists, err := resourceExists(ctx, client, token, "apps/v1", "Deployment", namespace, serviceName)
		if err != nil {
		...
		baseDeployment, err := fetchResourceAsMap(ctx, client, token, "apps/v1", "Deployment", namespace, serviceName)
		if err != nil {
			return fmt.Errorf("strategy reconcile: %w", err)
		}
		baseService, err := fetchResourceAsMap(ctx, client, token, "v1", "Service", namespace, serviceName)
		if err != nil {
			return fmt.Errorf("strategy reconcile: %w", err)
		}
		cleanResourceForReapply(baseDeployment)
		cleanResourceForReapply(baseService)
		...
		output, undoErr := runCommandWithInput(ctx, "kubectl", undoArgs, "")
		if undoErr != nil {
		...
		patchOutput, patchErr := runCommandWithInput(ctx, "kubectl", patchArgs, "")
		if patchErr != nil {
