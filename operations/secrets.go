...
func resolveEnvVars(ctx context.Context, cfg Config, ctxData deployContext, environment string) (map[string]string, map[string]string, error) {
...
func resolveSecretValue(ctx context.Context, cfg Config, ctxData deployContext, environment, value string) (string, error) {
...
func resolveVaultSecret(ctx context.Context, provider *secretProvider, ref string) (string, error) {
	address := strings.TrimSpace(stringValue(provider.Config, "address"))
	token := strings.TrimSpace(stringValue(provider.Config, "token"))
	if address == "" || token == "" {
...
func resolveAwsSecret(ctx context.Context, provider *secretProvider, ref string) (string, error) {
	accessKey := stringValue(provider.Config, "accessKeyId")
	secretKey := stringValue(provider.Config, "secretAccessKey")
	region := stringValue(provider.Config, "region")
	if region == "" {
		region = "us-east-1"
	}
...
func resolveGcpSecret(ctx context.Context, provider *secretProvider, ref string) (string, error) {
	serviceAccount := stringValue(provider.Config, "serviceAccountJson")
	projectID := stringValue(provider.Config, "projectId")
